...
public class ModifierRessource extends JFrame
{
	...
	public ModifierRessource(Ressource ressource)
	{
		...
		JLabel labelId = new JLabel("Code :");
		gbc.gridx = 0;
		gbc.gridy = 0;
		gbc.gridwidth = 1;
		...
		btnEnregistrer.addActionListener(e ->
		{
			...
			if (nouveauNom != null && !nouveauNom.trim().isEmpty() && nouvelId != null && !nouvelId.trim().isEmpty())
			{
				...
				if (ancienChemin.exists() && !nouveauChemin.exists())
				{
					...
					JOptionPane.showMessageDialog(this, "Ressource déjà existante", "Erreur", JOptionPane.ERROR_MESSAGE);
			}
			else
				JOptionPane.showMessageDialog(this, "Remplissez bien le nom et le code (ex: R1.01) de la ressource.", "Erreur", JOptionPane.ERROR_MESSAGE);
		});
