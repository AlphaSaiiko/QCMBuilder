...
function autoValidate() {
    if (!isValidationDone) {
        ...
        if (selectedAnswer) {
            ...
            popupText.innerHTML = '<span style="color: red;">Temps écoulé ! Aucune réponse sélectionnée.</span>';
            document.querySelector('.bonne-reponse').style.backgroundColor = 'green';
        }
        ...
        // Sauvegarder l'état
        sessionStorage.setItem(`isValidationDone-${questionId}`, true);
        sessionStorage.setItem(`selectedAnswer-${questionId}`, selectedAnswer ? selectedAnswer.getAttribute('id') : null);
        sessionStorage.setItem(`popupText-${questionId}`, popupText.innerHTML);
    }
}
